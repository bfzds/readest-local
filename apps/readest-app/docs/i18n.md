...
> ### 本分支的语言支持范围（readest-local，2026-09-18 起）
>
> 界面语言只维护三种：**简体中文 `zh-CN`（基准全集）、繁体中文 `zh-TW`（必须与简体键集对齐）、英文 `en`（只存复数形态与专名，其余靠键原文兜底）**。
>
> - **新增字符串**：只写 `zh-CN` 与 `zh-TW` 各一条；`en` 仅在需要正确单复数时补 `_one` / `_other`。`zh-TW` 必须同步——`src/__tests__/i18n/locale-key-diff.test.ts` 断言 `zh-TW ⊇ zh-CN`，并断言 `zh-CN` 覆盖源码里出现的每一条 `_()` 字面量（缺键时 i18next 回落键原文，中文界面会直接显示英文，而 `check:translations` 只查占位符、查不出这类缺口）。
> - **其余 31 个语言**（`i18n-langs.json` 里除 `zh-CN` / `zh-TW` 之外的条目）是从上游继承的既有权重资产：**保持原样、不新增、不维护**。它们的既有文案照旧可用，只是不含本分支新增的字符串（在这些语言下回落英文）。
> - **不要运行 `pnpm i18n:extract`**：本仓库里它会把判定为"源码中已不再使用"的键连同正在使用的复数条目一起删掉，并写入 `__STRING_NOT_TRANSLATED__` 占位符（实测一次运行改动 34 个语言文件、几万行 diff）。新键按上面的规则手工追加。
