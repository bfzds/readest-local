...
describe('getOrphanedBookEntries (#5837)', () => {
  ...
  it('reclaims everything but the kept sidecars in a soft-deleted book dir', async () => {
    ...
    // re-download resumes with them). A lingering book file, a feed's article
    // cache — and an `audiobook/` subdir, which upstream keeps for its paired
    // audiobook copies but this branch has no audiobook module for — are
    // leftovers to reclaim.
    const readDirectory = vi
      .fn()
      .mockResolvedValue(
        makeFiles(
    ...
    expect(entries).toEqual([
      ...
      { base: 'Books', path: `${DELETED}/audiobook/1-2-track.m4b`, size: 50 },
