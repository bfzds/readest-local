...
## 6. 与上游的关系：已彻底断开（2026-09-19）

本仓库是 Readest v0.12.1 的**硬分叉**：历史与上游**无共同祖先**（根提交是上游 0.12.1——上游提交
`f3e1df7e`——的快照导入），此后全部提交均为本地自建。上游不是本项目的**协作方、依赖方、服务方
或身份来源**，也没有任何自动机制会重建这些联系。

**已断开（2026-09-19，仓库元数据操作，不会体现在任何提交里，本节即唯一记录）**：
`upstream` 远程（含 push URL）已删除；211 个上游标签已删除并 `git gc`（.git 587M→365M）；
上游的 `.github` 资产（FUNDING / issue 模板 / CI workflow / dependabot）已删除；冒充上游身份的
元数据（Cargo `homepage` / `repository`、AppStream 的 URL / 截图 / developer id）已清除。

**判定规则（接手者照此执行，不必重新讨论）**：

- **必须保留（法律义务，不是"关系"）**：`LICENSE`（AGPL-3.0）、README 的来源声明、
  `Bilingify LLC` 署名（Cargo `authors`、About 窗口版权行）；对外分发构建产物时须提供对应源码。
- **保留（依赖或事实，不是关系）**：`.gitmodules` 里指向 `github.com/readest/*` 的依赖子模块；
  测试里解析 `readest.com` 分享链接（数据格式兼容）；代码注释里的上游 issue 编号（出处）；
  iOS/macOS 的 `com.bilingify.readest` 运行时标识（改动会破坏 Apple 签名与既有设备数据）。
- **禁止重建**：不得重新添加 `upstream` 或任何指向上游的 remote；不得恢复上游的 CI / 模板 /
  更新检查 / 遥测 / 云同步；不得引入任何会把上游资产自动带回来的机制。
- **允许的唯一上游通道**：显式、逐个、**手工移植**上游修复——临时 `git fetch <上游URL>` +
  `cherry-pick`（或手工搬运，如 `port/*`、`f` 分支的做法），移植内容照常过脱敏检查；
  移植完成后不留任何指向上游的持久引用。
