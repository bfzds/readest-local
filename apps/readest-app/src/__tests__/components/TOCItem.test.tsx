...
describe('current position indicator on the active item', () => {
  it('shows a small book icon beside the active chapter label', () => {
    const item = makeLeafItem({ href: 'chapter1.html' });
    const { container } = render(
      <StaticListRow
        {...defaultProps}
  ...
  it('does not show the icon on an inactive chapter', () => {
    const item = makeLeafItem({ href: 'chapter1.html' });
    const { container } = render(
      <StaticListRow
        {...defaultProps}
